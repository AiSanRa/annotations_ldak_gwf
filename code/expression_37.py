...
def process_data(variant_file):
    ...
    variant_data = pd.read_csv(variant_file, usecols=[0, 1], names=['Variant_ID', 'Gene_ID'])

    # Create the 'Annotations' folder if it doesn't exist
    output_folder = 'annot_37'
    ...
    for i, tissue in enumerate(exp_data.columns, start=1):
        ...
        # Group by Variant_ID and calculate the mean of the expression values for each gene
        aggregated_data = merged_data.groupby('Variant_ID')[tissue].mean().reset_index()

        # Standardize the TPMS values across all samples
        scaler = StandardScaler()
        aggregated_data[tissue] = scaler.fit_transform(aggregated_data[[tissue]])

        # Create the subfolder inside 'annot_37' with the name of the tissue
...
if __name__ == "__main__":
    variant_file_path = 'geneID_37.csv'
